...
class MainControllerClass {
            ...
            if (order1) {
                ...
                if (order2) {
                    ...
                    if (sendmail?.status) {
                        ...
                        if (processlog) {
                            return res.status(httpStatus.CREATED).send({ success: true, message: "Your Form has subbmitted successfully", data: order1 });
                        }
                    }
                }
            }
    ...
    uploadConstant = catchAsync(async (req: Request, res: Response) => {
        try {
            ...
            const constantfile = await mainService.uploadConstant(req.body.ORDER_ID, files);
            if (constantfile) {
                const getorderid = await mainService.getOrdereId(req.body.ORDER_ID);
                if (getorderid) {
                    const getemail = await mainService.getemailbyorder(getorderid?.ORDER_ID)
                    if (getemail) {
                        const sendmail: any = await mainService.UploadConsentEmail(getemail?.EMAIL, req.body.ORDER_ID);
                        if (sendmail) {
                            const processlog = await mainService.CreateProcessLog({
                            ...
                            if (processlog) {
                                return res.status(httpStatus.CREATED).send({ success: true, message: "Your consent is uploaded successfully", data: constantfile });
                            }
                        }
                    }
